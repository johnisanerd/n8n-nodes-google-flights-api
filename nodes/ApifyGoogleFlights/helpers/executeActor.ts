...
import { ACTOR_ID } from '../ApifyGoogleFlights.node';
import { buildActorInput } from '../ApifyGoogleFlights.properties';
...
/**
 * Shape a single flight option according to the chosen Output mode.
...
function shapeItem(
	...
	return {
		...
		stops: item.stops_label,
		travelClass: item.travel_class,
		flightNumbers: item.flight_numbers,
 ...
 * `all_flights` array. Flatten to one flight option per n8n item.
 */
function flattenFlights(items: any[]): Record<string, any>[] {
	const flights: Record<string, any>[] = [];
	for (const item of items) {
		const all = Array.isArray(item?.all_flights) ? item.all_flights : [];
		for (const f of all) {
			flights.push(f);
		}
	}
...
export async function runActor(this: IExecuteFunctions, i: number): Promise<INodeExecutionData[]> {
	...
	const flights = flattenFlights(items);
	const shaped = flights.map((flight) => shapeItem(flight, mode, fields));
